...
def getNewWH(ori_w, ori_h, dst_w=0, dst_h=0):
    '''输入原始宽高，目标宽或高，按比例计算出目标宽高'''
...
def resizeImg(img, dst_w=0, dst_h=0, qua=85):
    ...
    '''
    ori_w, ori_h = im.size
    newWidth,newHeight = getNewWH(ori_w, ori_h, dst_w, dst_h)
...
def getImageBase64(img):
    """输入：一个PIL Image对象。输出：该对象保存为png格式的文件的base64编码"""
    o = StringIO()
    img.save(o, "PNG")
    # img.resize((newWidth,newHeight),Image.ANTIALIAS).save(o, "JPEG", quality=60)
    return base64.b64encode(o.getvalue())
    Image.open(fp, mode)

def compressImageWithBase64(imgB64,dst_w=0):
    '''输入：一个图像的base64编码，缩小后的图像宽度。输出：把这个图像缩小后的base64编码'''
    fin = StringIO()
    fin.write(base64.b64decode(imgB64))
    img = Image.open(fin)
    ori_w, ori_h = im.size
    newWidth,newHeight = getNewWH(ori_w, ori_h, dst_w)
    fout = StringIO()
    img.resize((newWidth,newHeight),Image.ANTIALIAS).save(fout, "JPEG", quality=60)
