...
def runbasic(func, server_address=("0.0.0.0", 8080)):
  ...
  [cs]: http://www.owlfish.com/
    ...
    class WSGIHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            # 20150701 dongyg 判断如果路径是/x/./static/.也作为静态文件处理，因为SimpleHTTPServer.translate_path该函数会固定将工作目录添加到要获取的静态文件前，因此也修改了一下
            if self.path.startswith('/static/'):
                SimpleHTTPServer.SimpleHTTPRequestHandler.do_GET(self)
            elif self.path.startswith('/x/') and self.path.find('/static/')>0:
                self.path = self.path.replace('/x/','modules/')
                self.path = os.path.join(web.config.moduledir, self.path)
                SimpleHTTPServer.SimpleHTTPRequestHandler.do_GET(self)
            else:
                self.run_wsgi_app()
...
class StaticMiddleware:
    ...
    def __call__(self, environ, start_response):
        ...
        # 20150701 dongyg 判断如果路径是/x/./static/.也作为静态文件处理，因为StaticApp中用到了SimpleHTTPServer.translate_path该函数会固定将工作目录添加到要获取的静态文件前，因此也修改了一下
        if path.startswith(self.prefix):
            return StaticApp(environ, start_response)
        elif path.startswith('/x/') and path.find(self.prefix)>0:
            path = path.replace('/x/','modules/')
            environ["PATH_INFO"] = os.path.join(web.config.moduledir, path)
            return StaticApp(environ, start_response)
        else:
