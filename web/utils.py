...
def storify(mapping, *requireds, **defaults):
    ...
    for key in requireds + tuple(mapping.keys()):
        # print 'key:', repr(key)
        if isinstance(key,(str,unicode)) and key.strip() == '': #20160420 jincw: eliminate blank param names
            continue
        value = mapping[key]
        ...
        if isinstance(defaults.get(key), list) and not isinstance(value, list):
            value = [value]
        if isinstance(defaults.get(key), list) and (len(defaults.get(key))>0 and isinstance(defaults.get(key)[0],dict)): #20150922 dongyg: support web.input(f=[{}])
            value = mapping[key]
...
re_compile = re.compile # memoize(re.compile) #@@ threadsafe? # jincw 2016-02-14 no need
re_compile.__doc__ = """
...
def listget(lst, ind, default=None):
    ...
    Returns `lst[ind]` if it exists, `default` otherwise.
        ...
        'b'
    """
    if isinstance(lst,iterbetter): #20160720 dongyg
        lst = lst.list()
    if len(lst)-1 < ind:
        return default
    return lst[ind]
