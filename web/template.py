...
class Render:
    ...
    """
    def __init__(self, loc='templates', cache=None, base=None, pypkg=None, **keywords):
        self._loc = loc
    ...
    def _lookup(self, name):
        ...
        if os.path.isdir(path):
            ...
            if path:
                ...
                # 20160901 dongyg: try to load it from compiled html(__init__.pyc) when html file not exists.
                # GAE_Render has tow problems: 1.Import error when loc is realpath; 2.getattr don't get HTML Render when html file in subdirs like ../..
                # Beside, need to apperently use GAE_Render to create a Render.
                if self.pypkg:
                    try:
                        fname = name.replace(os.path.sep,'.').replace('/','.')
                        wname = self.pypkg.__name__+'.'+fname
                        pname = os.path.splitext(wname)[0] #Insure which import is fname's module. Import grandparent is useless.
                        exec 'import %s'%(pname)
                        mod = __import__(pname, None, None, ['x'])
                        mod.__dict__.update(self._keywords.get('builtins', TEMPLATE_BUILTINS))
                        mod.__dict__.update(Template.globals)
                        mod.__dict__.update(self._keywords.get('globals', {}))
                        eval(wname)
                        return 'pyc', fname
                    except Exception, e:
                        return 'none', None
    ...
    def _load_template(self, name):
        ...
        if kind == 'dir':
            ...
            # 20160901 dongyg: see above
            exec 'import %s'%(self.pypkg.__name__)
            return eval(self.pypkg.__name__+'.'+path)
        else:
            return None
            # raise AttributeError, "No template named " + name  #20160727 dongyg: don't raise
...
def compile_templates(root):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        # only .html/.htm/.xhtml/.xml
        filenames = [f for f in filenames if f.lower().endswith('.html') or f.lower().endswith('.htm') or f.lower().endswith('.xhtml') or f.lower().endswith('.xml')]
